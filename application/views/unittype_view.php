...
<body>
    ...
    <div class="container box-shadow">
        ...
        <h3>Unit Type</h3>
        <br />
        <button class="btn btn-success" onclick="add_unittype()"><i class="glyphicon glyphicon-plus"></i> Add New</button>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('unittype/ajax_list')?>",
            "type": "POST"
        },
...
function add_unittype()
{
    ...
    $('.modal-title').text('Add New Unit Type'); // Set Title to Bootstrap modal title
}

function edit_unittype(UnitTypeId)
{
    save_method = 'update';
    $('#form')[0].reset(); // reset form on modals    
    $('.form-group').removeClass('has-error'); // clear error class
    ...
    $.ajax({
        url : "<?php echo site_url('unittype/ajax_edit/')?>/" + UnitTypeId,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('[name="UnitTypeName"]').val(data.UnitTypeName);
...
function edit_del(UnitTypeId)
{
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('.modal-title').text('Delete UnitType'); // Set title to Bootstrap modal title
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('unittype/ajax_add')?>";
    } else {
        url = "<?php echo site_url('unittype/ajax_update')?>";
    }
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            else
            {
                ...
                { 
                    $('[UnitTypeName="'+data.inputerror[i]+'"]').parent().parent().addClass('has-error'); //select parent twice to select div form-group class and add has-error class
                    $('[UnitTypeName="'+data.inputerror[i]+'"]').next().text(data.error_string[i]); //select span help-block class set text error string
...
function save_del()
{
    ...
    $('#btnSave').attr('disabled',true); //set button disable 
    var url;
        url = "<?php echo site_url('unittype/ajax_update_del')?>";
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title">Add Unit Type Name</h3>
            </div>
            <div class="modal-body form">
                <form action="#" id="form" class="form-horizontal">
                    <input type="hidden" value="" name="UnitTypeId"/> 
                    ...
                    <div class="form-body">
                        <div class="form-group">
                            <label class="control-label col-md-3">Unit Type Name</label>
                            <div class="col-md-9">
                                <input name="UnitTypeName" placeholder="Unit Type Name" class="form-control" type="text">
