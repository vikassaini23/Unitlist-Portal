...
<body>
    ...
    <div class="container box-shadow">
        ...
        <h3>Donor Name</h3>
        <br />
        <button class="btn btn-success" onclick="add_donor()"><i class="glyphicon glyphicon-plus"></i> Add New</button>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('donor/ajax_list')?>",
            "type": "POST"
        },
...
function add_donor()
{
    ...
    $('.modal-title').text('Add New Donor'); // Set Title to Bootstrap modal title
}

function edit_donor(DonorId)
{
    save_method = 'update';
    $('#form')[0].reset(); // reset form on modals    
    $('.form-group').removeClass('has-error'); // clear error class
    ...
    $.ajax({
        url : "<?php echo site_url('donor/ajax_edit/')?>/" + DonorId,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('[name="DonorName"]').val(data.DonorName);
            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Edit Donor Name'); // Set title to Bootstrap modal title
...
function edit_del(DonorId)
{
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('.modal-title').text('Delete Donor Name'); // Set title to Bootstrap modal title
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('donor/ajax_add')?>";
    } else {
        url = "<?php echo site_url('donor/ajax_update')?>";
    }
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            else
            {
                ...
                { 
                    $('[DonorName="'+data.inputerror[i]+'"]').parent().parent().addClass('has-error'); //select parent twice to select div form-group class and add has-error class
                    $('[DonorName="'+data.inputerror[i]+'"]').next().text(data.error_string[i]); //select span help-block class set text error string
...
function save_del()
{
    ...
    $('#btnSave').attr('disabled',true); //set button disable 
    var url;
        url = "<?php echo site_url('donor/ajax_update_del')?>";
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title">Add Donor Name</h3>
            </div>
            <div class="modal-body form">
                <form action="#" id="form" class="form-horizontal">
                    <input type="hidden" value="" name="DonorId"/> 
                    ...
                    <div class="form-body">
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <input name="DonorName" placeholder="Donor Name" class="form-control" type="text">
...
<div class="modal fade" id="delete_form" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h3 class="modal-title">Donor Name</h3>
            </div>
            <div class="modal-body form">
