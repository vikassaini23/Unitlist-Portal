...
<body>
    ...
    <div class="container box-shadow">
        ...
        <h3>State</h3>
        <br />
        <button class="btn btn-success" onclick="add_state()"><i class="glyphicon glyphicon-plus"></i> Add New</button>
...
function add_state()
{
    ...
    $('.modal-title').text('Add New State'); // Set Title to Bootstrap modal title
}
...
function edit_state(StateId)
{
    ...
    $.ajax({
        url : "<?php echo site_url('state/ajax_edit/')?>/" + StateId,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('.modal-title').text('Edit State Name'); // Set title to Bootstrap modal title
...
function edit_del(StateId)
{
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('.modal-title').text('Delete StateName'); // Set title to Bootstrap modal title
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('state/ajax_add')?>";
    } else {
        url = "<?php echo site_url('state/ajax_update')?>";
    }
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            else
            {
                ...
                { 
                    $('[StateName="'+data.inputerror[i]+'"]').parent().parent().addClass('has-error'); //select parent twice to select div form-group class and add has-error class
                    $('[StateName="'+data.inputerror[i]+'"]').next().text(data.error_string[i]); //select span help-block class set text error string
...
function save_del()
{
    ...
    $('#btnSave').attr('disabled',true); //set button disable 
    var url;
        url = "<?php echo site_url('state/ajax_update_del')?>";
